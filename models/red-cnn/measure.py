...
def compute_CNR(img, label_map):
    """CNR = |mean(tissue) - mean(background)| / std(background).
    img and label_map must be numpy arrays in the same HU-scale coordinate system.
    label_map > 0 is treated as tissue ROI; == 0 is background.
    Returns 0.0 if either region is empty or background std is zero.
    """
    ...
    if roi_mask.sum() == 0 or bg_mask.sum() == 0:
        return 0.0
    bg_std = img[bg_mask].std()
    if bg_std == 0:
        return 0.0
    return float(abs(img[roi_mask].mean() - img[bg_mask].mean()) / bg_std)
