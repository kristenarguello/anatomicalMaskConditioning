...
class RED_CNN(nn.Module):
    def __init__(self, out_ch=96, in_channels=1):
        super(RED_CNN, self).__init__()
        self.conv1 = nn.Conv2d(in_channels, out_ch, kernel_size=5, stride=1, padding=0)
    ...
    def forward(self, x):
        # encoder — residual_1 takes only the first channel so the skip always matches 1-ch output
        residual_1 = x[:, :1, :, :]
