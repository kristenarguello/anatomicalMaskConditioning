...
class corediff(TrainTask):
    ...
    def set_model(self):
        ...
        self.use_mask = getattr(opt, "mask_dir", None) is not None
        
        denoise_fn = Network(in_channels=opt.in_channels, context=opt.context, use_masks=self.use_mask)
        ...
        if num_gpus > 1:
            print(f"Using DataParallel with {num_gpus} GPUs; "
                  f"batch {opt.batch_size} -> {opt.batch_size // num_gpus} per GPU")
    ...
    def reset_parameters(self):
        unwrap(self.ema_model).load_state_dict(unwrap(self.model).state_dict())
    ...
    def step_ema(self, n_iter):
        if n_iter < self.start_ema_iter:
            self.reset_parameters()
            return
        self.ema.update_model_average(unwrap(self.ema_model), unwrap(self.model))
    ...
    def train(self, inputs, n_iter):
        ...
        if self.use_mask:
            low_dose, full_dose, mask = inputs
            low_dose, full_dose, mask = low_dose.cuda(), full_dose.cuda(), mask.cuda()
        else:
        ...
        if opt.wandb:
            wandb.log({'epoch': n_iter, 'loss': loss_val})
    ...
    def test(self, n_iter):
        opt = self.opt
        ema = cast(Diffusion, unwrap(self.ema_model))
        ema.eval()

        psnr, ssim, rmse = 0., 0., 0.
        for batch in tqdm.tqdm(self.test_loader, desc="test", leave=False):
            if self.use_mask:
                low_dose, full_dose, mask = batch
                low_dose, full_dose, mask = low_dose.cuda(), full_dose.cuda(), mask.cuda()
            else:
            ...
            gen_full_dose, direct_recons, imstep_imgs = ema.sample(
    ...
    def generate_images(self, n_iter):
        ...
        if self.use_mask:
            low_dose, full_dose, mask = self.test_images # type: ignore
        else: 
            low_dose, full_dose = self.test_images # type: ignore
            mask = None
