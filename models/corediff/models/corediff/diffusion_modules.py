...
class Diffusion(nn.Module):
    ...
    @torch.no_grad()
    def sample(self, batch_size=4, img=None, t=None, sampling_routine='ddim', n_iter=1, start_adjust_iter=1, mask=None):
        assert self.denoise_fn is not None, "denoise_fn must be provided for sampling"
        assert img is not None, "initial image must be provided for sampling"
        ...
        if sampling_routine == 'ddim':
            while (t):
                ...
                if self.context:
                    full_img = torch.cat((up_img, img, down_img), dim=1) # type: ignore
                else:
                    full_img = img
                ...
                x1_bar = self.denoise_fn(full_img, step, x1_bar, noise, adjust=adjust, mask=mask)
    ...
    def forward(self, x, y, n_iter, only_adjust_two_step=False, start_adjust_iter=1, mask=None):
        '''
        ...
        '''
        assert self.denoise_fn is not None, "denoise_fn must be provided for forward pass"
        ...
        if only_adjust_two_step or n_iter < start_adjust_iter:
            x_recon = self.denoise_fn(x_mix, t, y, x_end, adjust=False, mask=mask)
        else:
            if t[0] == self.num_timesteps - 1:
                adjust = False
            else:
                adjust = True
            x_recon = self.denoise_fn(x_mix, t, y, x_end, adjust=adjust, mask=mask)
        ...
        if n_iter >= start_adjust_iter and t_single.item() >= 1:
            ...
            x_recon_sub1 = self.denoise_fn(x_mix_sub1, t_sub1, x_recon, x_end, adjust=True, mask=mask)
        else:
