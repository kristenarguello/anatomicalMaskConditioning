...
class Network(nn.Module):
    def __init__(self, in_channels=3, out_channels=1, context=True, use_masks=False):
        super(Network, self).__init__()
        self.use_mask = use_masks
        unet_in = in_channels + 1 if use_masks else in_channels
        self.unet = UNet(in_channels=unet_in, out_channels=out_channels)
        self.context = context

    def forward(self, x, t, y, x_end, adjust=True, mask=None):
        if self.context:
        ...
        if self.use_mask and mask is not None:
            x = torch.cat((x, mask), dim=1) # se a mascara existe, concatena ela na imagem
